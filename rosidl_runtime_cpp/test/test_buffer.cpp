...
using rosidl_runtime_cpp::Buffer;
using rosidl_runtime_cpp::BufferImplBase;
using rosidl_runtime_cpp::CpuBufferImpl;
...
// Test Buffer with shared_ptr semantics (copy shares impl)
TEST(TestBuffer, shared_impl) {
  Buffer<int> buffer1;
  buffer1.push_back(10);
  buffer1.push_back(20);
  
  Buffer<int> buffer2 = buffer1;  // Copy shares impl via shared_ptr
  
  // Get the impl pointers
  auto impl1 = buffer1.get_impl();
  auto impl2 = buffer2.get_impl();
  
  // They should point to the same implementation
  EXPECT_EQ(impl1.get(), impl2.get());
  EXPECT_EQ(impl1.use_count(), impl2.use_count());
}
...
TEST(TestBuffer, set_impl) {
  ...
  auto custom_impl = std::make_shared<CpuBufferImpl<int>>();
  ...
  // Inject the impl
  buffer.set_impl(custom_impl, "cpu");
